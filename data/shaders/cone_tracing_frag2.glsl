...
layout (binding = 6) uniform sampler3D tex3D;
...
vec4 SampleVoxels(vec3 World_Position, float diameter);
...
vec3 indirectDiffuse() {
    ...
    vec3 x = perp(y, vec3(0.0,0.5,1.0)); //1st side axis
...
//        vec3 sample_position = (pos_worldspace / vox_size) - nrm * 0.5f;
//        vec4 voxel = vol_sample(current_coneDiameter, sample_position);
        vec4 voxel = SampleVoxels(pos_worldspace, current_coneDiameter);
...
//    TEST_FRAG_COLOR.rgb = location;

    return textureLod(tex3D, location, vlevel);	// Sample
}
